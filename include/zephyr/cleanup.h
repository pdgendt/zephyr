...
#define DEFINE_CLASS(_name, _type, _exit, _init, ...)                                              \
	...
	static __maybe_unused inline void class_##_name##_destructor(_type *p)                     \
	{                                                                                          \
		_type _T = *p;                                                                     \
		_exit;                                                                             \
	}                                                                                          \
	static __maybe_unused inline _type class_##_name##_constructor(__VA_ARGS__)                \
	{                                                                                          \
		_type t = _init;                                                                   \
		return t;                                                                          \
	}
...
#define Z_DEFINE_DEFER_VOID(_func)                                                                 \
	/* The non-conditional variant */                                                          \
	DEFINE_CLASS(defer_##_func, void *, (void)_T; (void)_func(), NULL, void)                   \
	/* The conditional variant */                                                              \
	DEFINE_CLASS(defer_cond_##_func, bool, if (_T) { (void)_func(); }, cond, bool cond)
...
#define Z_DEFINE_DEFER_N(_func, ...)                                                               \
	...
	/* The conditional variant */                                                              \
	struct defer_cond_##_func##_ctx {                                                          \
		FOR_EACH_IDX(Z_DEFER_CTX_ARG, (), __VA_ARGS__)                                     \
		bool cond;                                                                         \
	};                                                                                         \
	DEFINE_CLASS(                                                                              \
		defer_cond_##_func, struct defer_cond_##_func##_ctx,                               \
		if (_T.cond) { (void)_func(FOR_EACH_IDX(Z_DEFER_EXIT_ARG, (, ), __VA_ARGS__)); },  \
		{FOR_EACH_IDX(Z_DEFER_INIT_ARG, (, ), __VA_ARGS__) Z_COMMA .cond = cond},          \
		FOR_EACH_IDX(Z_DEFER_FUNC_ARG, (, ), __VA_ARGS__), bool cond)
...
/**
 * @brief Use a conditional defer with automatic variable name
 *
 * This macro creates a conditional defer variable with an automatically generated unique name.
 * The defer will execute its cleanup action when going out of scope, but only if the
 * condition evaluated to true at initialization.
 *
 * This solves the problem of placing @ref DEFER inside an if statement, where the defer
 * would execute at the end of the if block rather than the end of the enclosing scope.
 * With DEFER_IF, the condition is evaluated once during initialization, and the cleanup
 * action is deferred until the end of the enclosing scope, executing only if the condition
 * was true.
 *
 * The conditional defer class is automatically defined alongside the regular defer class
 * when using @ref DEFINE_DEFER, so no separate definition is needed.
 *
 * @param _name Name of the defer type (defined by @ref DEFINE_DEFER or
 *              @ref DEFINE_CLASS with <tt>defer_</tt> prefix)
 *
 * Usage: @code{.c} DEFER_IF(k_free)(ptr, ret < 0); @endcode
 *
 * @see DEFER for unconditional defers
 */
#define DEFER_IF(_name) CLASS(defer_cond_##_name, Z_UNIQUE_ID(defer))
