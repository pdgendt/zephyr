...
{
	{
		...
		zexpect_false(called_void);
	}

	zexpect_true(called_void);
}

static int cond_value;
static void cond_function(int value)
{
	cond_value = value;
}
DEFINE_DEFER(cond_function, int)

ZTEST(cleanup_api, test_defer_cond_function)
{
	{
		DEFER_IF(cond_function)(67, false);
		zexpect_equal(cond_value, 0);
	}
	zexpect_equal(cond_value, 0);

	{
		DEFER_IF(cond_function)(42, true);
		zexpect_equal(cond_value, 0);
	}
	zexpect_equal(cond_value, 42);
}
