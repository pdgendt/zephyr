		       ...
		       int expected_status)
{
	...
	zassert_ok(k_sem_take(&connect_result_sem, TEST_EVENT_TIMEOUT),
		   "No connect result event");
...
ZTEST(wifi_emul, test_scan)
{
	...
	zassert_equal(scan_result_count, 2, "Expected 2 scan results, got %d",
		      scan_result_count);
}
...
ZTEST(wifi_emul, test_connect_psk)
{
	...
	zassert_ok(net_mgmt(NET_REQUEST_WIFI_IFACE_STATUS, wifi_iface, &status,
			    sizeof(status)));
...
{
	do_connect("SecureAP", "hunter22", WIFI_SECURITY_TYPE_PSK,
...
{
	do_connect("DoesNotExist", NULL, WIFI_SECURITY_TYPE_NONE,
...
ZTEST(wifi_emul, test_bridge_data_path)
{
	...
	zassert_mem_equal(hdr->src.addr, net_if_get_link_addr(wifi_iface)->addr,
			  WIFI_MAC_ADDR_LEN);
...
ZTEST(wifi_emul, test_self_contained_rx)
{
	...
	ret = packet_socket_recv(sock, buf, sizeof(buf), TEST_EVENT_TIMEOUT);
	zassert_equal(ret, sizeof(rx_payload), "No frame received on the Wi-Fi interface (%d)", ret);
	zassert_mem_equal(buf, rx_payload, sizeof(rx_payload));
